...
handleIntel()        → POST /intel → guarda item de Intel
botTick()            → Ejecuta tick del bot ficticio
render()             → Genera HTML del dashboard
...
| POST | `/intel` | Agrega item de Intel manual |
...
| GET | `/api/intel` | Items de Intel JSON |
...
- `bot.js` usa `CLAUDE_API_KEY` en lugar de `ANTHROPIC_API_KEY` (P6 pendiente)
- `bot.js` usa `claude-haiku-4-5` sin sufijo de fecha (P7 pendiente)
- `.gitignore` tiene entradas basura (`insertar`, `reemplazar`, `5`) (P8 pendiente)
- No hay `package.json` con versiones fijas de dependencias (aunque ya fue creado en F1)
